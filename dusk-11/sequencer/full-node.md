# Running a Dusk 11 Sequencer Full Node
...
sed -i'.bak' 's/persistent_peers = ""/persistent_peers = "f4b8a8dcfc5a142bd00aadab71f39dbfe7091d13@35.236.5.36:26656,ca3bc3562919b82575fe3ac5b11fa5962ce8cd3b@34.94.1.224:26656,4418000e355967ecc8e03004f5850dfde51c410b@34.102.98.161:26656,7a117e7906d8428ad20341aca94af03c980c11d8@34.102.109.86:26656"/g' ~/.cometbft/config/config.toml
```
...
```bash
curl -o genesis.json -s https://raw.githubusercontent.com/astriaorg/networks/main/dusk-11/sequencer/genesis.json
...
The first sequencer block which has a tx is `852` which has hash

```bash
B9B90E5D592ECDE990A2405327E7B00A53A64204DE9318061B29D6C128ECCA1D
```

### Check primary sequencer node

```bash
curl -X GET "https://rpc.sequencer.dusk-11.devnet.astria.org/block?height=852" \
...
```bash
curl -X GET "http://localhost:26657/block?height=852" \
  -H "accept: application/json" -s \
  | jq .result.block_id.hash
```
...
```bash
curl -o full-node-values.yaml -s https://raw.githubusercontent.com/astriaorg/networks/main/dusk-11/sequencer/full-node-values.yaml
```
...
helm install dusk11-full-node astria/astria-sequencer-validator --version 1.0.0-rc.1 \
  --namespace astria-dusk11-node --create-namespace -f full-node-values.yaml
