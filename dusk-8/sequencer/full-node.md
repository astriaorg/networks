# Running a Dusk 8 Sequencer Full Node
...
cd astria
git checkout tags/sequencer-v0.14.0
...
sed -i'.bak' 's/persistent_peers = ""/persistent_peers = "f4b8a8dcfc5a142bd00aadab71f39dbfe7091d13@35.235.122.184:26656,4418000e355967ecc8e03004f5850dfde51c410b@34.102.111.186:26656,ca3bc3562919b82575fe3ac5b11fa5962ce8cd3b@34.94.128.197:26656,7a117e7906d8428ad20341aca94af03c980c11d8@34.102.73.190:26656/g' ~/.cometbft/config/config.toml
```
...
```bash
curl -o genesis.json -s https://raw.githubusercontent.com/astriaorg/networks/main/dusk-8/sequencer/genesis.json
...
The first sequencer block which has a tx is `1460` which has hash

```bash
AA2A31D5F5E1353C23035D1F69C53DF004264314BAE3E53FB75FFDDD9D78DF8E
```

### Check primary sequencer node

```bash
curl -X GET "https://rpc.sequencer.dusk-8.devnet.astria.org/block?height=1460" \
...
```bash
curl -X GET "http://localhost:26657/block?height=1460" \
  -H "accept: application/json" -s \
  | jq .result.block_id.hash
```
...
```bash
curl -o full-node-values.yaml -s https://raw.githubusercontent.com/astriaorg/networks/main/dusk-8/sequencer/full-node-values.yaml
```
...
```bash
helm install dusk7-full-node astria/astria-sequencer-validator --version 0.14.0 \
